...
#################################################
	...
	similar_by: (vector, tolerance) ->
		-tolerance <= @x - vector.x <= tolerance and
		-tolerance <= @y - vector.y <= tolerance and
		-tolerance <= @z - vector.z <= tolerance
	...
	build_Debug_Support: () ->
		point = new THREE.Mesh( new THREE.SphereGeometry( 0.75, 16, 8 ),
...
#################################################
	...
	applyEuler: ( euler ) ->
		...
		if euler instanceof THREE.Euler == no
			console.error 'ERROR: Vector3\'s .applyEuler() now expects a Euler
				 rotation rather than a Vector3 and order.  Please update your code.'
