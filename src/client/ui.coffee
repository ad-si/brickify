module.exports = (globalConfig) ->
	return {
		...
		renderer: new THREE.WebGLRenderer( { alpha: true, antialiasing: true, preserveDrawingBuffer: true } )
		controls: null
		...
		# Bound to updates to the window size: Called whenever the window is resized.
		...
		init: ->
			...
			@renderer.domElement.addEventListener( 'drop', @dropHandler.bind( @ ), false )
			@fileReader.addEventListener( 'loadend', @loadHandler.bind( @ ), false )
			document.addEventListener( 'keyup', @keyUpHandler.bind( @ ) )
			window.addEventListener( 'resize', @windowResizeHandler.bind( @ ), false )
