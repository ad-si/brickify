...
export const exists = identifier => fetch(`./model/${identifier}`, { method: 'HEAD' })
  .then(response => {
...
const requestDataFromLocal = identifier => fetch(`./model/${identifier}`)
  .then(response => {
    if (!response.ok) throw new Error(`Model not found: ${identifier}`)
    return response.text()
  })

const buildModelPromise = identifier => requestDataFromLocal(identifier)
...
export const request = identifier => modelCache[identifier] != null
