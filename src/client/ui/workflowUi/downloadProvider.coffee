...
module.exports = class DownloadProvider
	...
	_createDownload: (fileType, selectedNode) =>
		...
		if (fileType == 'stl')
			...
			_paq.push(['trackEvent', 'Editor', 'ExportAction', 'StudRadius', downloadOptions.studRadius])
			_paq.push(['trackEvent', 'Editor', 'ExportAction', 'HoleRadius', downloadOptions.holeRadius])
