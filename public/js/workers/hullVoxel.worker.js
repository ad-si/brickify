// Standalone worker for hull voxelization

const HullVoxelWorker = {
  voxelize (model, lineStepSize, floatDelta, voxelRoundingThreshold, progressCallback) {
  ...
  _interpolateLine ({start: {x: x1, y: y1, z: z1}, end: {x: x2, y: y2, z: z2}}, i) {
    i = Math.min(i, 1.0)
    const x = x1 + ((x2 - x1) * i)
    const y = y1 + ((y2 - y1) * i)
    const z = z1 + ((z2 - z1) * i)
  ...
  _voxelizeLine (a, b, direction, stepSize, grid) {
    ...
    let currentVoxel = {x: 0, y: 0, z: -1}
    const currentGridPosition = { x: a.x, y: a.y, z: a.z }

    for (let i = 0, end = length, step = stepSize; i <= end; i += step) {
      if (!this._isOnVoxelBorder(currentGridPosition)) {
        ...
        if ((oldVoxel.x !== currentVoxel.x) || (oldVoxel.y !== currentVoxel.y) || (oldVoxel.z !== currentVoxel.z)) {
          const z = this._getGreatestZInVoxel(a, b, currentVoxel)
  ...
  _getGreatestZInVoxel (a, b, {x, y, z}) {
    ...
    if (aIsInVoxel && bIsInVoxel) return Math.max(a.z, b.z)
    if (aIsInVoxel && (a.z > b.z)) return a.z
    if (bIsInVoxel && (b.z > a.z)) return b.z
    ...
    if (d.z !== 0) {
      const minZ = z - 0.5
      k = (minZ - a.z) / d.z
      if (k >= 0 && k <= 1) return minZ
      const maxZ = z + 0.5
      k = (maxZ - a.z) / d.z
      if (k >= 0 && k <= 1) return maxZ
    }
  },
  ...
  _setVoxel ({x, y, z}, zValue, direction, grid) {
    ...
    if (oldValue) {
      if (((direction !== 0) && (zValue > (oldValue.z + this.floatDelta))) || ((direction === 1) && (zValue > (oldValue.z - this.floatDelta)))) {
        oldValue.z = zValue
  ...
  _postProgress (progressFloat, callback) {
    ...
    if (!(currentProgress > this.lastProgress)) return
    this.lastProgress = currentProgress
    callback({ state: 'progress', progress: currentProgress })
  },

  _forEachPolygon (model, visitor) {
    const { faceVertexIndices, coordinates, directions } = model
    const { length } = directions
    for (let i = 0; i < length; i++) {
      const i3 = i * 3
      const p0 = { x: coordinates[faceVertexIndices[i3] * 3], y: coordinates[(faceVertexIndices[i3] * 3) + 1], z: coordinates[(faceVertexIndices[i3] * 3) + 2] }
      const p1 = { x: coordinates[faceVertexIndices[i3 + 1] * 3], y: coordinates[(faceVertexIndices[i3 + 1] * 3) + 1], z: coordinates[(faceVertexIndices[i3 + 1] * 3) + 2] }
      const p2 = { x: coordinates[faceVertexIndices[i3 + 2] * 3], y: coordinates[(faceVertexIndices[i3 + 2] * 3) + 1], z: coordinates[(faceVertexIndices[i3 + 2] * 3) + 2] }
      const direction = directions[i]
      visitor.call(this, p0, p1, p2, direction, i / length)
    }
  },
}
...
self.onmessage = (event) => {
  ...
  if (type === 'voxelize') {
    ...
    try {
      HullVoxelWorker.voxelize.call(HullVoxelWorker, model, lineStepSize, floatDelta, voxelRoundingThreshold, progressCallback)
    } catch (err) {
