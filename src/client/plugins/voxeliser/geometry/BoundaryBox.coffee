...
class BoundaryBox
		...
		for y in [0...dim.y] by 1
			...
			for z in [0...dim.z] by 1
				...
				for x in [0...dim.x] by 1
					...
					voxels[y][z][x] = new BoundaryBox(x_value, y_value, z_value,
						x_value + Lego.width, y_value + Lego.height, z_value + Lego.width )
		voxels
	...
	build_Lego: (top = yes, bottom = yes, front, back, left, right) ->
		...
		middle_point = {x: (@minX + @maxX) / 2, y: (@minY + @maxY) / 2, \
		z: (@minZ + @maxZ) / 2}
		...
		for angle in [0..360] by circle_steps
			x = middle_point.x + Math.sin(angle * Math.PI / 180.0) * Lego.knob_radius
			y = @maxY
			z = middle_point.z + Math.cos(angle * Math.PI / 180.0) * Lego.knob_radius
			circle_top.push new CSG.Vector(x, y, z)

		circle_cap = []
		for angle in [0..360] by circle_steps
			x = middle_point.x + Math.sin(angle * Math.PI / 180.0) * Lego.knob_radius
			y = @maxY + Lego.knob_height
			z = middle_point.z + Math.cos(angle * Math.PI / 180.0) * Lego.knob_radius
			circle_cap.push new CSG.Vector(x, y, z)
		...
		for angle in [360..0] by -circle_steps
			...
			y = @minY
			z = middle_point.z  + Math.cos(angle * Math.PI / 180.0) * Lego.knob_radius
			circle_bottom.push new CSG.Vector(x, y, z)

		circle_inlet = []
		for angle in [360..0] by -circle_steps
			x = middle_point.x + Math.sin(angle * Math.PI / 180.0) * Lego.knob_radius
			y = @minY + Lego.knob_height
			z = middle_point.z + Math.cos(angle * Math.PI / 180.0) * Lego.knob_radius
			circle_inlet.push new CSG.Vector(x, y, z)

		#build faces
		#faces = []

		faces  = [ [ [ points[0], points[4], points[5], points[1] ],
								 new CSG.Vector( 0, 0,-1), {origin: ['lego'], side: '-z'} ],
							 [ [ points[3], points[2], points[6], points[7] ],
								 new CSG.Vector( 0, 0, 1), {origin: ['lego'], side: '+z'} ],
							 [ [ points[0], points[3], points[7], points[4] ],
								 new CSG.Vector(-1, 0, 0), {origin: ['lego'], side: '-x'} ],
		...
		for side in [0...4]
			...
			top_faces.push [ [ old_top_edge, edge_top_point , old_top_point ],
											 new CSG.Vector(0,1, 0), {origin: ['lego'], side: '+y'}]
			bottom_faces.push [[old_bottom_edge, edge_bottom_point, old_bottom_point],
												new CSG.Vector(0,-1, 0), {origin: ['lego'], side: '-y'}]

			beginning = side * point_per_segement + 1
			end = (side + 1) * point_per_segement
			for point in [beginning .. end]
				...
				top_faces.push [ [ current_top_point, old_top_point, edge_top_point ],
												 new CSG.Vector( 0, 1, 0), {origin: ['lego'], side: '+y'} ]
				cap_faces.push [ [ current_top_point, current_cap_point, old_cap_point,
													 old_top_point ],
												 null, {origin: ['lego'], side: '+y'} ]

				bottom_faces.push [ [ current_bottom_point, old_bottom_point,
															edge_bottom_point ],
														new CSG.Vector( 0, -1, 0), {origin: ['lego'], side: '-y'} ]
				inlet_faces.push [ [ current_bottom_point, current_inlet_point,
														 old_inlet_point, old_bottom_point ],
													 null, {origin: ['lego'], side: '-y'} ]
		...
		###if top
			...
			faces.push [ [ points[2], points[1], points[5], points[6] ],
   new CSG.Vector( 1, 0, 0), {origin: ['cut'], axis: 'x', vec: 1 } ]

		if bottom
			something
		else
			faces.push [ [ points[0], points[3], points[7], points[4] ],
   new CSG.Vector(-1, 0, 0), {origin: ['cut'], axis: 'x', vec: -1} ]
		###

		faces.push [ circle_cap[0...circle_cap.length - 1], new CSG.Vector(0,1,0),
			{origin: ['lego'], side: '+y'} ]
		faces.merge bottom_faces
		faces.push [circle_inlet[0...circle_inlet.length - 1], new CSG.Vector(0,1,0)
		, {origin: ['lego'], side: '-y'} ]
