const HullVoxelWorker = {
  ...
  voxelize(model, lineStepSize, floatDelta, voxelRoundingThreshold, progressCallback) {
    ...
    this._forEachPolygon(model, function(p0, p1, p2, direction, progress) {
      ...
      return self2._postProgress(progress, progressCallback);
    });
    progressCallback({
      state: "finished",
      data: grid
  ...
  _setVoxel({ x, y, z }, zValue, direction, grid) {
    ...
    if (oldValue) {
      if (direction !== 0 && zValue > oldValue.z + this.floatDelta || // Prefer setting direction to 1 (i.e. close the voxel)
